...
class LengthRule(BaseRule):
    ...
    def _process_answer(self, answer: Answer) -> Result:
        ...
        passed = True
        feedback = f"The answer length is {answer_length}."
        if self.min_length is not None and answer_length < self.min_length:
            passed = False
            feedback += f" It is shorter than the minimum length of {self.min_length}."
        if self.max_length is not None and answer_length > self.max_length:
            passed = False
            feedback += f" It is longer than the maximum length of {self.max_length}."
